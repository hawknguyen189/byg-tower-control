...
// export function getBscScanLink(networkId, data, type) {
//   const prefix = `${NetScanUrlPrefix[networkId] || NetScanUrlPrefix[56]}`;
...
//   }
// }
export function beautifyNumber(amount, decimal = 0) {
  if (amount !== null) {
    var parts = amount.toString().split(".");
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    if (decimal > 0 && parts.length > 1) {
      return (
        parts[0] +
        "." +
        (parts[1] + "00000000000000000000000000000").slice(0, decimal)
      );
    } else if (decimal > 0) {
      return parts[0] + "." + "00000000000000000000000000000".slice(0, decimal);
    } else {
      return parts.join(".");
    }
  }
}
export function toSequenceNumter(num) {
  var val = parseInt(num);
  if (val === 1) return "first";
  else if (val === 2) return "second";
  else if (val === 3) return "third";
  else return val + "th";
}
...
export function getTxErrorMsg(err) {
  return err?.data?.message || err?.message || "Transaction failed!";
}
...
export const fetchShipDetails = async (shipURI) => {
  const respone = await fetch(shipURI);
